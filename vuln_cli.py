...
from src.lib.cli_options import cli_args
...
def cli(args=cli_args()):
    ...
    if args.summary:
        print_color(f"[*] Getting the summary of MS vulnerabilities for {args.date}\n")
        sleep(secs)
        print(vuln_type_summary(get_msrc(date_string=args.date)))
    if args.criticals:
        print_color(f"[*] Getting the critical MS vulnerabilities for {args.date}\n")
        sleep(secs)
        print(parse_criticals(get_msrc(date_string=args.date)))
    if args.zero_days:
        print_color(f"[*] Getting the zero day MS vulnerabilities for {args.date}\n")
        sleep(secs)
        print(parse_zero_days(get_msrc(date_string=args.date)))
    if args.browsers:
        print_color(f"[*] Getting the Edge browser vulnerabilities for {args.date}\n")
        sleep(secs)
