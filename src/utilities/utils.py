...
def is_dead_zone():
    def find_patch_tuesday(month=date.today().month, year=date.today().year):
        ...
        # The datetime module starts the week on Mon (0) by default,
        # so if the 12th day of the month lands on a Sunday(6) and
        # we add 1 then we get 7 which would throw our script off.
        # To offset this we will set any day of the week provided by
        # the datetime module that is greater than 6 to 0 (or Sunday).
    ...
    date_list = pd.date_range(
        ...
        end=patch_tuesday.date() - timedelta(days=1)
...
def make_dataframe(data_frame_list):
...
def validate_cve(cve_id):
    url = import_config(get_config().as_posix()).get(
        "settings").get("urls").get("nvd")
